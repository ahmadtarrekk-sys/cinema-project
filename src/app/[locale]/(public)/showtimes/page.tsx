...
export default async function GlobalShowtimesPage({ searchParams, params }: ShowtimesPageProps) {
  ...
  // Generate date options for the picker (Today + 7 days = rolling window)
  ...
  return (
    <div className="min-h-screen pt-24 pb-24">
      {/* Background */}
      <div className="fixed inset-0 min-h-screen bg-background -z-10" />
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12 flex flex-col gap-6 md:flex-row md:items-end justify-between">
          <div>
            <h1 className="font-display text-4xl font-bold tracking-tight text-foreground mb-2">Showtimes</h1>
                  ...
                  <Button
                    ...
                    className={`h-auto min-w-24 flex-col gap-1 rounded-2xl py-3 px-4 transition-all ${
                      isSelected 
                        ...
                        : "bg-cinema-surface border-border text-muted-foreground hover:border-gold/30 hover:text-foreground"
                    }`}
                  >
        ...
        <div className="space-y-8">
          {moviesWithShowtimes.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-3xl border border-border bg-cinema-surface py-24 text-center">
               <Calendar className="h-16 w-16 text-muted-foreground/20 mb-6" />
               <h3 className="text-2xl font-semibold text-foreground mb-2">No Showtimes Found</h3>
            ...
            moviesWithShowtimes.map(({ movie, showings }) => {
              ...
              return (
                <div key={movie.id} className="overflow-hidden rounded-3xl border border-border bg-cinema-surface shadow-xl">
                  <div className="grid md:grid-cols-[300px_1fr]">
                    
                    {/* Movie Info Sidebar */}
                    <div className="relative p-6 border-b md:border-b-0 md:border-r border-border bg-foreground/5 flex flex-col md:flex-row md:items-start gap-4">
                         ...
                         {movie.posterUrl ? (
                            <>
                              ...
                              <img 
                                src={movie.posterUrl} 
                                alt={title} 
                                className="w-24 md:w-full aspect-[2/3] object-cover rounded-xl shadow-lg border border-border group-hover:border-gold/50 transition-colors"
                              />
                            </>
                         ) : (
                            <div className="w-24 md:w-full aspect-[2/3] bg-foreground/5 rounded-xl border border-border flex items-center justify-center group-hover:border-gold/50 transition-colors">
                       ...
                       <div className="flex-1 min-w-0">
                         <Link href={`/movies/${movie.id}`} className="hover:underline hover:text-gold block">
                           <h2 className="text-xl md:text-2xl font-bold text-foreground mb-1 truncate md:whitespace-normal leading-tight">{title}</h2>
                         </Link>
                         ...
                         <div className="flex items-center gap-2 text-xs text-muted-foreground">
                           ...
                           {movie.rating && <span className="border border-border px-1.5 py-0.5 rounded text-[10px] uppercase font-bold text-muted-foreground">{movie.rating}</span>}
                         </div>
                       </div>
                    </div>
                    ...
                    <div className="p-6 md:p-8">
                       <h3 className="text-sm font-semibold text-foreground uppercase tracking-widest mb-6 opacity-60">Schedule</h3>
                       <div className="space-y-6">
                         {showings.map(st => {
                            ...
                            return (
                               <div key={st.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-xl border border-border bg-foreground/[0.02] hover:bg-foreground/[0.04] transition-colors">
                                 <div className="flex items-start sm:items-center gap-4">
                                   ...
                                   <div>
                                     <div className="flex items-center gap-1.5 text-foreground font-medium mb-1 truncate">
                                 ...
                                 <Link href={`/book/${st.id}`} className="shrink-0">
                                   <Button className="w-full sm:w-auto bg-foreground/10 text-foreground hover:bg-gold hover:text-black font-semibold ring-1 ring-border hover:ring-transparent transition-all">
                                     Book Ticket
                                   </Button>
                                 </Link>
                               </div>
